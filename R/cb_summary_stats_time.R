...
cb_summary_stats_time <- function(df, .x, digits = 2) {
  ...
  # ===========================================================================
  # All NA check
  # If all values of .x are NA, then populate the table in an informative way.
  # Else calculate the min, mode, and max of non-missing values.
  # ===========================================================================
  unqiue_nonmissing_vals <- unique(df[[.x]])[!is.na(unique(df[[.x]]))]
  if (length(unqiue_nonmissing_vals) == 0) {
    summary <- data.frame(
      Statistic = c("Minimum", "Mode", "Maximum"),
      Value = "All values are missing",
      Frequency = length(df[[.x]]),
      Percentage = 100.00
    )
    ...
    # ===========================================================================
    counts <- df %>%
      # Filter out missing so that `NA` isn't returned as the mode value.
